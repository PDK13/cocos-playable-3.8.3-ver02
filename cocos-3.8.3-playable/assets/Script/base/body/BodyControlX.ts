...
export class BodyControlX extends Component {
    ...
    protected onLoad(): void {
        ...
        director.on(BaseConstant.INPUT_JUMP, this.onJump, this);
        director.on(BaseConstant.INPUT_JUMP_RELEASE, this.onJumRelease, this);

        director.on(BaseConstant.INPUT_MOVE_UP, this.onMoveUp, this);
        director.on(BaseConstant.INPUT_MOVE_DOWN, this.onMoveDown, this);
        director.on(BaseConstant.INPUT_MOVE_LEFT, this.onMoveLeft, this);
        director.on(BaseConstant.INPUT_MOVE_RIGHT, this.onMoveRight, this);
        director.on(BaseConstant.INPUT_MOVE_RELEASE, this.onMoveRelease, this);
        director.on(BaseConstant.INPUT_MOVE_RELEASE_X, this.onMoveReleaseX, this);
        director.on(BaseConstant.INPUT_MOVE_RELEASE_Y, this.onMoveReleaseY, this);

        director.on(BaseConstant.PLAYER_BODY_SLEEP, this.onSleep, this);
        director.on(BaseConstant.PLAYER_BODY_AWAKE, this.onAwake, this);

        director.on(BaseConstant.PLAYER_COMPLETE, this.onComplete, this);
        ...
        if (this.m_bodyAttack != null)
            director.on(BaseConstant.PLAYER_ATTACK_UP, this.m_bodyAttack.onMeleeAttackUp, this.m_bodyAttack);

        if (this.m_bodyAttack != null)
            director.on(BaseConstant.INPUT_FIRE, this.onAttack, this);

        director.on(BaseConstant.INPUT_INTERACTION, this.onInteraction, this);
    ...
    protected start(): void {
        ...
        if (this.UiPickBtnActive)
            director.emit(BaseConstant.INPUT_INTERACTION_SHOW, false);
    }
    ...
    onInteractionPickAndThrow() {
        ...
        if (this.m_pickUp == null) {
            ...
            //Node Event
            this.m_pickUp.emit(BaseConstant.ON_NODE_PICK);
            //Animation
            delayPick = this.m_bodySpine.onPick();
            this.scheduleOnce(() => this.m_bodySpine.onPickLoop(), this.m_bodySpine.onPick());
            //Ui
            director.emit(BaseConstant.INPUT_INTERACTION_ICON, this.UiThrowIconIndex);
        }
        else {
            this.m_pickUpProgess = true;
            //Node Event
            this.m_pickUp.emit(BaseConstant.ON_NODE_THROW);
            ...
            if (this.UiPickBtnActive)
                director.emit(BaseConstant.INPUT_INTERACTION_SHOW, false);
            director.emit(BaseConstant.INPUT_INTERACTION_ICON, this.UiPickIconIndex);
        }
        if (this.m_pickUpProgess)
    ...
    onInteractionFound(target: Node, stage: boolean) {
        ...
        if (this.UiPickBtnActive)
            director.emit(BaseConstant.INPUT_INTERACTION_SHOW, true);
        director.emit(BaseConstant.INPUT_INTERACTION_ICON, this.UiPickIconIndex);
    }
    ...
    protected onComplete(Centre: Vec3) {
        ...
        director.emit(BaseConstant.INPUT_LOCK);
    ...
    private onCompleteProgess() {
        ...
        this.scheduleOnce(() => {
            this.scheduleOnce(() => {
                director.emit(BaseConstant.GAME_COMPLETE);
    ...
    protected onDead() {
        ...
        this.scheduleOnce(() => {
            ...
            this.scheduleOnce(() => {
                ...
                director.emit(BaseConstant.GAME_LOSE);
